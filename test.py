...
class TriviaTestCase(unittest.TestCase):
    ...
    # check when data base is empty, before running \
    # psql test_db < capstone.psql
    ...
    def test_add_destination(self):
        res = self.client().post('/countries/1/\
        add_destination', json={'destinationId': 2})
        data = json.loads(res.data)
    ...
    def test_404_if_country_does_not_exist(self):
        res = self.client().delete('/countries/\
        1000', json={'destinationId': 2})
        data = json.loads(res.data)
    ...
    def test_admin_can_add_destination(self):
        head = {'Authorization': 'Bearer ' + os.environ.get('ADMIN_TOKEN')}
        res = self.client().post('/countries/1/\
        add_destination', json={'destinationId': 2}, headers={head})
        data = json.loads(res.data)
    ...
    def test_visitor_cant_add_destination(self):
        head = {'Authorization': 'Bearer ' + os.environ.get('VISITOR_TOKEN')}
        res = self.client().post('/countries/1/\
        add_destination', json={'destinationId': 2}, headers={head})
        data = json.loads(res.data)
    ...
    def test_admin_can_delete_destination(self):
        head = {'Authorization': 'Bearer ' + os.environ.get('ADMIN_TOKEN')}
        res = self.client().delete('/countries/\
        1', json={'destinationId': 2}, headers={head})
        data = json.loads(res.data)
    ...
    def test_visitor_cant_delete_destination(self):
        head = {'Authorization': 'Bearer ' + os.environ.get('VISITOR_TOKEN')}
        res = self.client().delete('/countries/\
        1', json={'destinationId': 2}, headers={})
        data = json.loads(res.data)
